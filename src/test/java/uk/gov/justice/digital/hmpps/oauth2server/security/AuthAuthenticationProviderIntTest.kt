...
class AuthAuthenticationProviderIntTest {
  ...
  fun authenticate_AuthUserSuccessWithAuthorities() {
    ...
    assertThat(auth.authorities).extracting<String> { it.authority }.containsOnly("ROLE_OAUTH_ADMIN", "ROLE_MAINTAIN_ACCESS_ROLES", "ROLE_MAINTAIN_OAUTH_USERS")
  }
  ...
  fun authenticate_AuthUserLockAfterThreeFailures() {
    assertThatThrownBy { provider.authenticate(UsernamePasswordAuthenticationToken("AUTH_TEST", "wrong")) }.isInstanceOf(BadCredentialsException::class.java)
    assertThatThrownBy { provider.authenticate(UsernamePasswordAuthenticationToken("AUTH_TEST", "wrong")) }.isInstanceOf(BadCredentialsException::class.java)
    assertThatThrownBy { provider.authenticate(UsernamePasswordAuthenticationToken("AUTH_TEST", "wrong")) }.isInstanceOf(LockedException::class.java)
  }

  @Test
  fun authenticate_AuthUserResetAfterSuccess() {
    assertThatThrownBy { provider.authenticate(UsernamePasswordAuthenticationToken("AUTH_USER", "wrong")) }.isInstanceOf(BadCredentialsException::class.java)
    assertThatThrownBy { provider.authenticate(UsernamePasswordAuthenticationToken("AUTH_USER", "wrong")) }.isInstanceOf(BadCredentialsException::class.java)
  ...
  fun authenticate_ExpiredUserWrongPassword() {
    assertThatThrownBy { provider.authenticate(UsernamePasswordAuthenticationToken("EXPIRED_USER", "wrong")) }.isInstanceOf(BadCredentialsException::class.java)
  }
}
